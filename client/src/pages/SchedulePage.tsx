...
export default function SchedulePage() {
  ...
  async function refresh() { const { data } = await axios.get('/schedules'); setSchedules(data.schedules) }
  useEffect(() => { refresh() }, [])
  async function create(e: React.FormEvent) {
    e.preventDefault()
    await axios.post('/schedules', { athleteId, title, startTime, endTime })
    setTitle(''); setStartTime(''); setEndTime(''); refresh()
  }
  async function addToCalendar(id: string) {
    await axios.post(`/schedules/${id}/calendar`, { accessToken })
